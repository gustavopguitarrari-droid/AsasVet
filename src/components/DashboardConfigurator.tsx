...
const DashboardConfigurator: React.FC<DashboardConfiguratorProps> = ({
  ...
  const handleAddCardToPanel = (id: string) => {
    setTempConfig((prevConfig) =>
      prevConfig.map((item) =>
        item.id === id ? { ...item, isVisible: true, category: item.category || "recentActivity" } : item // Adiciona à categoria padrão se não tiver uma
      )
    );
  };
                  ...
                  {availableCards.map((item) => (
                    ...
                    >
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleAddCardToPanel(item.id)}
